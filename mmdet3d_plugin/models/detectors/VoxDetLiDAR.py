...
def run_length_positive(t, dim):
    ...
    for i in range(L - 2, -1, -1):
        ...
        val = torch.where(
            cond, out_next + 1, torch.tensor(1, dtype=torch.int32, device=t.device)
        )
        out[tuple(idx)] = val
...
def compute_all_direction_distances(gt_occ):
    ...
    distances = torch.stack(
        [dist_x_pos, dist_x_neg, dist_y_pos, dist_y_neg, dist_z_pos, dist_z_neg], dim=1
    )
    return distances
...
class VoxDetLiDAR(BaseModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.voxel_backbone = build_from_cfg(lidar_backbone, MODELS)
        self.voxel_neck = build_from_cfg(lidar_neck, MODELS)
    ...
    def forward_train(self, data_dict):
        ...
        if self.use_gt_refine:
            ...
            if self.car_scale_filter_min is not None:
                ...
                car_mask_min = (
                    x_mask_min & y_mask_min & z_mask_min
                ).bool() & cls_mask  # all thre axes are one
            ...
            if self.global_scale_filter_min is not None:
                ...
                mask_min_g = (
                    x_mask_min_g & y_mask_min_g & z_mask_min_g
                )  # isolated points
        ...
        output = self.pts_bbox_head(
            voxel_feats=[[voxel_feats_enc[0][0]], [voxel_feats_enc[1][0]]],
            img_metas=img_metas,
            img_feats=None,
            gt_occ=gt_occ,
        )
